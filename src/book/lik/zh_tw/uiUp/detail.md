    ...
    stage.shiftBtn = FrameButton(kit .. "->shiftBtn", stage.main)
        ...
        function(evtData)
            ...
            if (detailType == "data") then
                detailType = "intro"
                evtData.triggerFrame:text("檢視數值")
                evtData.triggerFrame:texture("btn/red")
            else
                detailType = "data"
                evtData.triggerFrame:text("檢視說明")
...
function this:attackMode(whichUnit)
    ...
    if (am == nil) then
        ...
        elseif (am:mode() == "missile") then
            if (am:homing()) then
                mode = mode .. colour.hex(colour.gold, "[武器]可跟蹤型遠端箭矢")
            else
                mode = mode .. colour.hex(colour.gold, "[武器]非跟蹤型遠端箭矢")
            end
...
function this:intro(key)
    local data = {
        ...
        [SYMBOL_ODD .. "hurtRebound"] = "反傷的機率",
        ["reborn"] = "重生時間",
        ["cure"] = "影響生命魔法恢復",
        ["stun"] = "攻擊時的眩暈時間",
        [SYMBOL_ODD .. "stun"] = "攻擊時的眩暈機率",
        ["crit"] = "攻擊時的暴擊傷害加成",
        [SYMBOL_ODD .. "crit"] = "攻擊時的暴擊機率",
...
function this:refresh(whichUnit)
    ...
    if (isClass(whichUnit, UnitClass)) then
        ...
        elseif (detailType == "intro") then
            stage.attackMode:text("說明單位攻擊模式|n包含戰鬥模式、箭矢屬性、附魔等資訊")
            for i = 1, 4 do
                local d = {}
                for _, a in ipairs(as[i]) do
                    if (a == '') then
